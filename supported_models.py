...
class SD20(supported_models_base.BASE):
    ...
    def model_type(self, state_dict, prefix=""):
        if self.unet_config["in_channels"] == 4: #SD2.0 inpainting models are not v prediction
            ...
            # JAF Berechnung der populationsbasierten Varianz (entspricht unbiased=False)
            var_out = torch.mean((out - mean_out) ** 2)
            std_out = torch.sqrt(var_out)

            if std_out > 0.09:
            # JAF if torch.std(out, unbiased=False) > 0.09: # not sure how well this will actually work. I guess we will find out.
